    ...
    return (
      <div className="glass p-6 shadow-xl rounded-xl">
        <div className="flex items-center space-x-4">
          ...
          <div>
            <h3 className="font-bold text-gray-900 dark:text-white mb-1">No Live Matches Currently</h3>
            <p className="text-sm text-gray-600 dark:text-gray-400">Matches will appear here when they&apos;re in progress</p>
          </div>
        </div>
      </div>
    );
    ...
    <div className="glass p-6 shadow-2xl rounded-2xl relative overflow-hidden">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 relative z-10">
        {matches.map((match, index) => (
          <div key={index} className="glass p-5 rounded-xl shadow-lg hover:shadow-xl transition-all duration-300 border border-white/20 dark:border-white/10">
            <div className="flex items-center justify-between mb-4">
              ...
              <span className="text-sm font-bold text-gray-700 dark:text-gray-300 bg-white/50 dark:bg-black/30 px-2 py-1 rounded">{match.minute}&apos;</span>
            </div>
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                <span className="font-semibold text-gray-900 dark:text-white">{match.homeTeam}</span>
                <span className="text-3xl font-black bg-gradient-to-br from-primary to-primary-600 bg-clip-text text-transparent">{match.homeScore}</span>
              </div>
              <div className="h-px bg-gradient-to-r from-transparent via-gray-300 dark:via-gray-700 to-transparent"></div>
              <div className="flex items-center justify-between">
                <span className="font-semibold text-gray-900 dark:text-white">{match.awayTeam}</span>
                <span className="text-3xl font-black bg-gradient-to-br from-primary to-primary-600 bg-clip-text text-transparent">{match.awayScore}</span>
              </div>
            </div>
          </div>
        ))}
